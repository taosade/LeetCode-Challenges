...
{
	const secretChars: { [key: string]: number } = {}

	for (const i in [...secret])
		if (secret[i] !== guess[i])
			secretChars[secret[i]!] = (secretChars[secret[i]!] ?? 0) + 1
	...
	{
		if (guess[i] === secret[i]) { bulls++; continue }

		if (!secretChars[guess[i]!]) continue

		cows++; secretChars[guess[i]!]--
	}
